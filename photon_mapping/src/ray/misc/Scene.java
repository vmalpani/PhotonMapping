...
public class Scene {
    ...
    protected ArrayList<Surface> specularSurfaces = new ArrayList<Surface>();

    /** luminaires are not just triangles but surfaces */
    protected ArrayList<Surface> luminairesAsSurfaces = new ArrayList<Surface>();
    ...
    private AccelerationStructure photonAccel;
    ...
    public void addSurface(Surface toAdd) {
        ...
        //our luminaires seen as whole surfaces, not just triangles
        if(((toAdd instanceof Mesh) || (toAdd instanceof Sphere)) && toAdd.getMaterial().isEmitter())
        {
        	luminairesAsSurfaces.add(toAdd);
        }
        Color specRef = toAdd.getMaterial().getSpecular(null).getReflectance();
        if (specRef.r > 0.00001 || specRef.g > 0.00001 || specRef.b > 0.00001 || toAdd.getMaterial().getRefractiveIndex() > Ray.EPSILON)
        {
        	this.specularSurfaces.add(toAdd);
        }
    }
    ...
    /**
     * Adding new method which returns the list of spherical luminaires
     */
    
    public ArrayList<Surface> getSphericalLights()
    {
    	ArrayList<Surface> sphericalLights = new ArrayList<Surface>();
    	for (Surface surface : this.luminaires)
    		if (surface instanceof Sphere)
    			sphericalLights.add(surface);
    	return sphericalLights;
    }
    
    /**
     *  Return the luminaires as surefaces (meshes, spehres or triangles...)
     */
    public ArrayList<Surface> getLuminairesAsSurfaces()
    {
    	return luminairesAsSurfaces;
    }
    
    public ArrayList<Surface> getSpecularSurfaces()
    {
    	return this.specularSurfaces;
    }
