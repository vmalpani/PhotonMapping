...
public class Mesh extends Surface {
    ...
    private double[] triangleAreasCumulative;
    ...
    private void setMeshData(float[] verts, int[] tris, float[] normals, float[] texcoords) {
        ...
        this.triangleAreasCumulative = new double[numTriangles];
        this.verts = verts;
        ...
        this.triangleAreasCumulative[0] = 0;
    	for (int i = 1; i < numTriangles; i++)
    	{
    		MeshTriangle tri = this.triangles[i - 1];
    		this.triangleAreasCumulative[i] = this.triangleAreasCumulative[i - 1] + tri.getArea();  
    	}
    	this.area = this.triangleAreasCumulative[this.numTriangles - 1] + this.triangles[this.numTriangles - 1].getArea();
    	
    	this.boundingBox = new AxisAlignedBoundingBox();
    	for (int i = 0; i < this.triangles.length; i++)
    	{
    		this.triangles[i].addToBoundingBox(this.boundingBox);    		
    	}        
    }
    ...
    public boolean chooseSamplePoint(Point3 iRec, Point2 seed, LuminaireSamplingRecord lRec) {
    	...
    	double randomArea = Math.random() * this.getArea();
    	int randomTri = findFirstIndexGreaterThan(1, this.numTriangles - 1, randomArea);
    	if (randomTri != -1)
    		return this.triangles[randomTri].chooseSamplePoint(iRec, seed, lRec);
    	return false;        
    }
    
    private int findFirstIndexGreaterThan(int low, int high, double key)
    {
    	if (key > this.triangleAreasCumulative[high])
    		return high;
    	if (key < this.triangleAreasCumulative[low])
    		return 0;
    	while (low < high)
    	{
    		int mid = (high + low) / 2;
    		if (this.triangleAreasCumulative[mid] < key)
    		{
    			if (this.triangleAreasCumulative[mid + 1] >= key)
    				return mid;
    			low = mid + 1;    			    			
    		}
    		else
