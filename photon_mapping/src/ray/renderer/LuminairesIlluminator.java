    ...
    public void directIllumination(Scene scene, Vector3 incDir, Vector3 outDir, 
        ...
        if ( scene.chooseVisiblePointOnLuminaire(seed, iRec, lumRec) ) {
            ...
            if ( brdf != null ) {
				...
				iRec.surface.getMaterial().getSpecular(iRec).evaluate(iRec.frame, incDir, outDir, specBRDF);
				brdfVal.add(specBRDF);
                outColor.scale(brdfVal);
                outColor.scale(-lumRec.iCosine*lumRec.lCosine/(dist*dist*lumRec.pdf));    // one extra "dist" to normalize consine values
