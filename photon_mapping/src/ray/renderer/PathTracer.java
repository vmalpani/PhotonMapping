...
public abstract class PathTracer extends DirectOnlyRenderer {
    ...
    public void gatherIllumination(Scene scene, Vector3 outDir, 
    	...
    	for(int sampleIndex2 = 0; sampleIndex2 < sampler.getNumSamples(); sampleIndex2++) {
            ...
            //sampler.sample(1, sampleIndex2, seed);
            sampler.sample(0, sampleIndex2, seed);
            ...
            if ( brdf != null ) {
            	...
            	ray.makeOffsetRay();
            	rayRadianceRecursive(scene, ray, sampler, sampleIndex2, level + 1, rayColor);
            	rayColor.scale(brdfVal);
            	rayColor.scale(Math.PI);
